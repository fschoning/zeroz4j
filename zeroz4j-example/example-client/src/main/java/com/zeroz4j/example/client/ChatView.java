...
public class ChatView extends Card {
    ...
    public ChatView() {
        ...
        WasmRmiClient.registerPushListener("chat.new_message", msg -> {
            refreshHistory();
        });
        WasmRmiClient.registerPushListener("chat.cleared", msg -> {
            refreshHistory();
        });
    }
    ...
    private void refreshHistory() {
        new Thread(() -> {
            try {
                ...
                messagesSignal.set(history);
                errorMessageSignal.set("");
            } catch (Exception ex) {
                System.err.println("[zeroz4j] Chat error: " + ex.getMessage());
                errorMessageSignal.set("Failed to load chat history: " + ex.getMessage());
            }
        }).start();
    }
}
