...
* **Serializable payloads** — the value type must be wire-serializable (`@Portable` or a `BinarySerializer`-supported type). Treat shared values as immutable: `set()` a new instance, never mutate the current one.
